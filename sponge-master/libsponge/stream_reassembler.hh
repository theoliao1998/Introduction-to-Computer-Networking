...
struct Packet {
  ...
  Packet(const uint64_t _index, const std::string _data, const bool _eof)
    : index(_index), data(_data), eof(_eof), add(0) {}

};

struct cmp_Packet{
	bool operator()(Packet a, Packet b) const {
    if(a.index == b.index){
...
class StreamReassembler {
  private:
    ...
    std::set<Packet, cmp_Packet> _tmp;
    size_t tmp_size;
